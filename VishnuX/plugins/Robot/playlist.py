...
from VishnuX.utils.database import (delete_playlist, get_playlist,
                                       get_playlist_names,
                                       save_playlist)
from VishnuX.utils.decorators.language import language, languageCB
...
async def check_playlist(client, message: Message, _):
    ...
    msg = _["playlist_4"]
    count = 0
    for Vishnu in _playlist:
        _note = await get_playlist(message.from_user.id, Vishnu)
        title = _note["title"]
        title = title.title()
...
async def add_playlist(client, CallbackQuery, _):
    ...
    try:
        ...
        return await CallbackQuery.message.reply_text(
            _["playlist_10"].format(CallbackQuery.from_user.mention),
        )
    except:
        return
